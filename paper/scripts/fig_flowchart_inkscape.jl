...
plot_raw = plot3d(dat_raw[1, :], dat_raw[2, :], dat_raw[3, :],
        color=COLOR_DICT["data_uncontrolled"], lw=2, legend=false,
        titlefontsize=48,
...
let d = sindy_dat_unctr
        global plot_uncontrolled1 = plot(d[1, ind],
                ...
                xticks=false, yticks=false, zticks=false, legend=false)
end
    plot!(dat[1, ind], color=COLOR_DICT["true"], lw=2)
...
let d = sindy_dat_unctr
    ...
    plot!(dat[2, ind], color=COLOR_DICT["true"], lw=2)
...
plot_subsample = plot(dat[plot_coordinate,ind],
                    ...
                    legend=false, xticks=false, yticks=false, lw=3)
    ylabel!("X", guidefontsize=24)
    title!("Subsampled data", titlefontsize=28)
...
plot_partial = plot(sindy_grad_ctr[plot_coordinate,ind],
                    color=COLOR_DICT["model_partial"],
                    legend=false, xticks=false, yticks=false, lw=8)
    plot!(dat_grad[plot_coordinate,ind],
        color=COLOR_DICT["true"],
        legend=false, xticks=false, yticks=false, lw=2)
    ylabel!("dX/dt", guidefontsize=24)
    title!("Partial model data", titlefontsize=28)
...
plot_ctr_guess = plot(ctr_guess[plot_coordinate,ind],
                    ...
                    xticks=false, yticks=false, lw=6)
    plot!(U_true[plot_coordinate,ind],
            color=COLOR_DICT["true"], legend=false,
            xticks=false, yticks=false, lw=3)
...
let d = sindy_dat_ctr
    ...
    title!("Revealed Underlying System", titlefontsize=28)
